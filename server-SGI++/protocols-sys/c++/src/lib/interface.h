...
extern "C" {
    ...
    // 加密server端的每一行
    SerialCT server_aggregation(const ClientFHE* cfhe, uint64_t a_dim, uint64_t hidden_dim, const uint64_t* const* matrix);
    // 解密AX - r
    uint64_t** server_decrypt_ax(const ClientFHE* cfhe, uint64_t a_dim, uint64_t hidden_dim, SerialCT ct);

    // sfhe 同态加密的公钥
    // a_dim 邻接矩阵A的维度
    // hidden_dim 特征矩阵X的特征向量数
    // client_vector client端的随机向量
    // ct X + client_vector的序列化密文
    // a_matrix 邻接矩阵A
