...
use protocols::{neural_network::{NNProtocol, ClientState, ServerState}, AdditiveShare};
...
pub fn client_connect(
    ...
    for _ in 0..16 {
        ...
        let stream = match stream {
            Ok(s) => s,
            Err(err) => {
                TcpStream::connect(addr).expect("仍然出错！")
            },
        };
...
pub fn nn_client_offline<R: RngCore + CryptoRng>(
    server_addr: &str,
    ...
    add_to_trace!(|| "Client Offline Communication", || format!(
        "Read {} bytes\nWrote {} bytes",
...
pub fn nn_server_offline<R: RngCore + CryptoRng>(
    server_addr: &str,
    ...
    add_to_trace!(|| "Server Offline Communication", || format!(
        "Read {} bytes\nWrote {} bytes",
        offline_read, offline_write
    ));
    server_state
...
pub fn nn_client_online<R: RngCore + CryptoRng>(
    client_state: &ClientState<TenBitExpParams>,
    server_addr: &str,
    architecture: &NeuralArchitecture<TenBitAS, TenBitExpFP>,
    ...
    add_to_trace!(|| "Client Online Communication", || format!(
        "Read {} bytes\nWrote {} bytes",
...
pub fn nn_server_online<R: RngCore + CryptoRng>(
    server_state: &ServerState<TenBitExpParams>,
    server_addr: &str,
    nn: &NeuralNetwork<TenBitAS, TenBitExpFP>,
    rng: &mut R,
) -> u64{
    ...
    add_to_trace!(|| "Server Online Communication", || format!(
        "Read {} bytes\nWrote {} bytes",
        online_read, online_write
    ));
...
pub fn nn_server<R: RngCore + CryptoRng>(
    ...
    let online_time = timer_start!(|| "online time");
    let (_, online_read, online_write) = {
        let (mut reader, mut writer) = server_connect(server_addr);
        (
            NNProtocol::online_server_protocol(&mut reader, &mut writer, &nn, &server_state)
