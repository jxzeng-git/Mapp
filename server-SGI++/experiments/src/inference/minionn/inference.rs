...
fn main() {
    ...
    std::fs::File::open(Path::new("/home/zjx/SecureNNI/improved-add/rust/data/X_10_7.npy"))
        .unwrap()
        .read_to_end(&mut buf)
        .unwrap();
    ...
    buf = vec![];
    std::fs::File::open(Path::new("/home/zjx/SecureNNI/improved-add/rust/data/A_10.npy"))
        .unwrap()
        .read_to_end(&mut buf)
        .unwrap();
    let image_vec: Vec<f64> = NpyData::from_bytes(&buf).unwrap().to_vec();
